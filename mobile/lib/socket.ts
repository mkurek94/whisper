...
const SOCKET_URL = "https://whisper-1-3pch.onrender.com";
...
export const useSocketStore = create<SocketState>((set, get) => ({
  ...
  connect: (token, queryClient) => {
    ...
    socket.on("socket-error", (error: { message: string }) => {
      ...
      Sentry.logger.info("Socket error", { message: error.message });
    });
    ...
    socket.on("new-message", (message: Message) => {
      ...
      //   Update chat's lastMessages directly for instant UI update
  ...
  sendMessage: (chatId, text, currentUser) => {
    ...
    const errorHandler = (error: { message: string }) => {
      Sentry.logger.info("Failed to send message", {
        chatId,
        error: error.message,
      });
